...
export function sanitizeChunk(raw) {
  ...
  // If it's only whitespace/newlines, ignore (SSE padding / keep-alives)
  if (!text.replace(/\n/g, "").trim()) {
    return "";
  }

  // Trim trailing spaces (nice), but DO NOT trim left or collapse internal spaces/newlines
  return text.replace(/[ \t]+$/g, "");
}

export function finalizeForRender(text) {
  if (!text) return "";
  // Safe cleanup at the *end* of the message; do not be aggressive.
